...
import { sendPaymentSentEmail, sendPaymentReceivedEmail, sendSwapCompleteEmail } from './emailService.js';
...
export async function setupUserTrustlines(user) {
  if (!user.stellarSecretKey) {
    console.warn(`⚠️  User ${user.id} has no secret key. Skipping trustline setup.`);
    return false;
  }
  ...
  try {
    ...
    console.error(`❌ Trustline setup failed for user ${user.id}:`, err.message);
    return false;
  }
}
...
export async function sendAsset(fromUserId, toAddress, amount, assetCode, memo = "") {
  const asset = assetCode === "XLM" ? StellarSdk.Asset.native() : ASSETS[assetCode];
  ...
  const keypair = StellarSdk.Keypair.fromSecret(decrypt(sender.stellarSecretKey));
  let destinationPublicKey = toAddress;
  let destinationUsername = null;

  if (!/^G[A-Z0-9]{55}$/.test(toAddress)) {
    const username = toAddress.replace("@dayfi.me", "").replace("@", "").toLowerCase();
  ...
  if (destinationUsername) {
    const recipient = await prisma.user.findUnique({ where: { username: destinationUsername } });
    if (recipient?.id) {
  ...
  try {
    // Email to sender
    await sendPaymentSentEmail(sender.email, destinationUsername || destinationPublicKey, amount, assetCode, memo);

    // Email to recipient (if it's a dayfi user)
    if (destinationUsername) {
      const recipient = await prisma.user.findUnique({ where: { username: destinationUsername } });
      if (recipient?.email) {
        await sendPaymentReceivedEmail(recipient.email, sender.username || 'Someone', amount, assetCode, memo);
      }
    }
  } catch (err) {
...
export async function swapAssets(fromUserId, fromAssetCode, toAssetCode, amount) {
  const sender = await prisma.user.findUnique({ where: { id: fromUserId } });
  if (!sender?.stellarPublicKey || !sender?.stellarSecretKey)
    throw new Error("Stellar wallet not found");
  ...
  const xlmBalance = parseFloat(account.balances.find(b => !b.asset_type || b.asset_type === 'native')?.balance || '0');
  const numTrustlines = account.balances.filter(b => b.asset_type === 'credit').length;
  console.log(`📋 Account state: Sequence=${account.sequenceNumber()}, XLM=${xlmBalance}, Trustlines=${numTrustlines}`);
  console.log(`   Balances: ${account.balances.map(b => `${b.balance} ${b.asset_code || 'XLM'}`).join(' | ')}`);

  const sendAsset = fromAssetCode === "XLM" ? StellarSdk.Asset.native() : ASSETS[fromAssetCode];
  const destAsset = toAssetCode === "XLM" ? StellarSdk.Asset.native() : ASSETS[toAssetCode];

  console.log(`🔄 SWAP: Finding path ${fromAssetCode} (${amount}) -> ${toAssetCode}`);

  // 1. Get Path for destMin
  const paths = await server.strictSendPaths(sendAsset, amount.toString(), [destAsset]).call();
  if (!paths.records.length) {
    console.error(`❌ No liquidity path found: ${fromAssetCode} -> ${toAssetCode} for ${amount}`);
  ...
  console.log(`✅ Path found: ${path.length} hops, Will receive ~${bestPath.destination_amount} ${toAssetCode}`);
  if (path.length > 0) {
    console.log(`   Intermediate assets: ${path.map(a => a.code || 'XLM').join(' → ')}`);
  }
  ...
  console.log(`💰 Fee: ${txFeeInXLM} XLM, Trustlines: ${numTrustlines} existing + ${newTrustlinesNeeded} new`);
  console.log(`🔐 Reserve needed: 0.5 × (1 + ${numTrustlines} + ${newTrustlinesNeeded}) = ${totalReserveNeeded.toFixed(8)} XLM`);
  console.log(`   Min total needed: ${minXLMRequired.toFixed(8)} XLM, Have: ${xlmBalance.toFixed(8)} XLM`);
  
  if (xlmBalance < minXLMRequired) {
    throw new Error(`Insufficient XLM for swap with this path. Need: ${minXLMRequired.toFixed(8)} XLM (${newTrustlinesNeeded} new trustlines × 0.5 + fee), Have: ${xlmBalance.toFixed(8)} XLM. Try swapping a smaller amount or get more XLM.`);
  }
  ...
  try {
    ...
    console.error(`❌ STELLAR REJECTED: ${horizonResponse?.title || 'Unknown Error'}`);
    console.error('Stellar extras:', JSON.stringify(horizonResponse?.extras || {}, null, 2));
    ...
    if (horizonResponse?.extras?.result_codes) {
      ...
      throw new Error(`Stellar rejected: tx_code=${txCode}, op_codes=[${opCodes.join(', ')}]`);
    }
    throw horizonErr;
  }
  ...
  const actualReceivedRaw = resultXDR.result().results()[0].tr().pathPaymentStrictSendResult().success().last().amount().toString();
  // Convert stroops to XLM/USDC (1 unit = 10^7 stroops)
  const actualReceived = parseFloat(actualReceivedRaw) / 10000000;

  console.log(`✅ Swap submitted: Hash ${result.hash} | Received: ${actualReceived} ${toAssetCode}`);

  // 3. Record accurately in DB
  await prisma.transaction.create({
    data: {
      userId: fromUserId,
      type: "swap",
      ...
      stellarTxHash: result.hash,
      memo: `received:${actualReceived.toFixed(6)} ${toAssetCode}`,
      isSwap: true,
  ...
  try {
    await sendSwapCompleteEmail(sender.email, fromAssetCode, toAssetCode, parseFloat(amount), actualReceived.toFixed(6));
  } catch (err) {
...
export async function resolveUsername(username) {
  const lower = username.replace("@dayfi.me", "").replace("@", "").toLowerCase();
...
export async function getStellarTransactions(publicKey, limit = 20) {
  try {
    const page = await server.transactions().forAccount(publicKey).limit(limit).order("desc").call();
...
export async function sendFromMasterWallet(recipientAddress, amount, memo = "") {
  const masterPublicKey = process.env.MASTER_WALLET_PUBLIC_KEY;
  const masterSecretEncrypted = process.env.MASTER_WALLET_SECRET_KEY;
...
export async function syncBlockchainTransactions(userId) {
  ...
  try {
    ...
    for (const tx of page.records) {
      ...
      for (const op of ops) {
        if (op.type_code === 'payment' || op.type_code === 'path_payment_strict_send') {
          // Only record transactions TO this user
          if (op.to === user.stellarPublicKey || op.destination === user.stellarPublicKey) {
