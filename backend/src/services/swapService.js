...
const GOLD_ISSUER = process.env.GOLD_ISSUER || 'GDBE77Z976GZ66WQSBCYI3S7A67T5OVCB57FPR35CCV72L7DQXNGA476';
...
const ENCRYPTION_KEY = Buffer.from(process.env.WALLET_ENCRYPTION_KEY || 'a'.repeat(64), 'hex');
...
function getSwapCategory(fromAsset, fromNetwork, toAsset, toNetwork) {
  ...
  const bothEVM = ['ethereum', 'arbitrum', 'polygon', 'avalanche'].includes(fromNetwork) &&
...
async function getStellarSwapQuote(fromAsset, toAsset, sellAmount) {
  ...
  if (!from || !to) throw new Error(`Stellar DEX: unsupported pair ${fromAsset}/${toAsset}`);
  ...
  try {
    ...
    if (!paths.records.length) throw new Error('No liquidity path found on Stellar DEX');
...
async function executeStellarSwap({ userId, fromAsset, toAsset, fromAmount, minReceive, path }) {
  const user = await prisma.user.findUnique({ where: { id: userId } });
  if (!user?.stellarPublicKey || !user?.stellarSecretKey) throw new Error('Stellar wallet not found');
  ...
  return { hash: result.hash, provider: 'stellar_dex', fromAsset, toAsset, fromAmount };
}
...
async function get0xQuote({ fromToken, toToken, sellAmount, network, takerAddress }) {
...
async function getChangellyQuote({ fromCurrency, toCurrency, fromAmount }) {
  const [minAmount, exchangeAmount] = await Promise.all([
    changellyRequest('getMinAmount', { from: fromCurrency.toLowerCase(), to: toCurrency.toLowerCase() }),
  ...
  return {
    ...
    toAsset: toCurrency,
    fromAmount,
    toAmount: parseFloat(Array.isArray(exchangeAmount) ? exchangeAmount[0]?.result : exchangeAmount),
    minAmount: parseFloat(minAmount),
    rate: parseFloat(Array.isArray(exchangeAmount) ? exchangeAmount[0]?.result : exchangeAmount) / fromAmount,
...
async function executeChangellySwap({ fromCurrency, toCurrency, fromAmount, toAddress, refundAddress }) {
...
export async function getSwapQuote({ fromAsset, fromNetwork, toAsset, toNetwork, fromAmount, userId }) {
  ...
  switch (category) {
    ...
    case 'evm_0x': {
      ...
      const quote = await get0xQuote({ fromToken, toToken, sellAmount, network: fromNetwork, takerAddress: user.evmPublicKey });
      return {
        provider: '0x',
    ...
    case 'cross_chain_changelly':
      return await getChangellyQuote({ fromCurrency: fromAsset, toCurrency: toAsset, fromAmount });
...
export async function executeSwap({ userId, fromAsset, fromNetwork, toAsset, toNetwork, fromAmount, quoteData }) {
  const category = getSwapCategory(fromAsset, fromNetwork, toAsset, toNetwork);

  switch (category) {
    case 'stellar_dex':
      return await executeStellarSwap({
        userId, fromAsset, toAsset, fromAmount,
        minReceive: quoteData?.toAmount,
        path: quoteData?.path,
      });

    case 'evm_0x':
      throw new Error('EVM swap execution via 0x requires frontend permit2 signing — use rawQuote.transaction');

    case 'cross_chain_changelly': {
      const user = await prisma.user.findUnique({ where: { id: userId } });
      const toAddress = toNetwork === 'stellar'
      ...
      const refundAddress = fromNetwork === 'stellar'
        ? user.stellarPublicKey
        : fromNetwork === 'bitcoin'
          ? user.btcPublicKey
          : user.evmPublicKey;
...
export function getSwapRouteInfo(fromAsset, fromNetwork, toAsset, toNetwork) {
  ...
  return {
    category,
    provider: category === 'stellar_dex' ? 'Stellar DEX' : category === 'evm_0x' ? '0x Protocol' : 'Changelly',
    estimatedTime: category === 'stellar_dex' ? '~5 seconds' : category === 'evm_0x' ? '~30 seconds' : '~10-30 minutes',
    feeTier: category === 'stellar_dex' ? 'Very low' : category === 'evm_0x' ? 'Low–Medium' : 'Medium (0.5%)',
  };
}
