...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      try {
        await sendOTP(email, otp, isNewUser);
      } catch (err) {
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const { publicKey, encryptedSecretKey, encryptedMnemonic } =
      ...
      // After wallet creation: fund the account
      const fundingResult = await fundNewUserWallet(publicKey, decoded.userId);
      if (!fundingResult) {
        console.error(
          `❌ Auto-funding failed — account may not be created on-chain`,
        );
      }

      // Wait for Stellar ledger to close (~7-10 seconds after funding)
      await new Promise((r) => setTimeout(r, 10000));

      // Fetch user object for trustline setup
      const user = await prisma.user.findUnique({
        where: { id: decoded.userId },
      });

      // Retry trustline setup with retries
      let trustlineSuccess = false;
      for (let attempt = 1; attempt <= 3; attempt++) {
        try {
          trustlineSuccess = await setupUserTrustlines(user);
          if (trustlineSuccess) {
            console.log(`✅ Trustlines set up on attempt ${attempt}`);
            break;
          }
        } catch (err) {
          console.error(
            `❌ Trustline setup attempt ${attempt} error:`,
            err.message,
          );
        }
        if (attempt < 3) {
          console.log(
            `⏳ Trustline attempt ${attempt} failed, retrying in 5s...`,
          );
      ...
      if (!trustlineSuccess) {
        console.warn(
          `⚠️  Trustlines not set up for ${publicKey} — will need retry`,
        );
      }
